...
<xsl:template match="css" mode="htmlhead">
	...
	<link rel="stylesheet" type="text/css" href="{$cssfile}?r={$release}" media="screen" />
</xsl:template>
...
<xsl:template match="favicon" mode="htmlhead">
	...
	<link rel="shortcut icon" href="{$iconfile}?r={$release}" />
</xsl:template>
...
<xsl:template match="script" mode="htmlhead">
	...
	<script type="text/javascript" src="{$jsfile}?r={$release}">&#xa0;</script> <!-- {$config/system/content_domain} -->
</xsl:template>
...
<xsl:template name="inline.js">
	...
	<xsl:choose>
		<xsl:when test="contains($localstr, '|')">
			<script type="text/javascript" src="{substring-before($localstr, '|')}?r={$release}">
		...
		<xsl:when test="$localstr != ''">
			<script type="text/javascript" src="{$localstr}?r={$release}">&#xa0;</script>
		</xsl:when>
	</xsl:choose>
</xsl:template>
...
<xsl:template name="inline.css">
	...
	<xsl:if test="$localstr != ''">
		<xsl:choose>
			<xsl:when test="contains($localstr, '|')">
				<link rel="stylesheet" type="text/css" href="{substring-before($localstr, '|')}?r={$release}">
					<xsl:call-template name="inline.attrs">
						<xsl:with-param name="str" select="substring-after($localstr, '|')"/>
					</xsl:call-template>
				</link>
			...
			<xsl:when test="$localstr != ''">
				<link rel="stylesheet" type="text/css" href="{$localstr}?r={$release}" />
			</xsl:when>
		</xsl:choose>
	</xsl:if>
</xsl:template>
