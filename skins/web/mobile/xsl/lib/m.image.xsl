...
<xsl:template name="image.bucket.lazy">
	...
	<img alt="{$alt}">
		...
		<!-- <xsl:attribute name="height">56.4%</xsl:attribute> -->
		...
		<xsl:choose>
			<xsl:when test="$m_image/@size = 'bomb'">
				<div class="big">
					...
					<div class="container">
						<h1 itemprop="headline"><xsl:value-of select="$title" /></h1>
					</div>
				</div>
			</xsl:when>
...
<xsl:template match="m_gallery_embed">
	...
	<div id="{@item_id}-swiper" class="swiper m_gallery_embed">
		...
		<div class="swiper-wrapper">
			...
			<xsl:for-each select="/xml/content/*/multimedias/images/image">
				...
				<xsl:if test="contains($isSelected, '1')">
					<div class="swiper-slide m_image">
						<figure>
							<xsl:call-template name="image.bucket">
								<xsl:with-param name="id" select="@image_id" />
							...
							<figcaption>
								...
								<xsl:if test="credit != ''">
									<span class="credit">Foto: <xsl:value-of select="credit" /></span>
								</xsl:if>
								<xsl:comment />
							</figcaption>
						</figure>
	...
	<!-- <script>
		slideSwiper = $('.swiper').swiper({ 
			slidesPerView: 1,
			nextButton: '.next', 
			prevButton: '.prev',
			setWrapperSize: true,
			centeredSlides: true, 
			slideActiveClass: 'showing'
	    });
 	</script> -->
</xsl:template>
