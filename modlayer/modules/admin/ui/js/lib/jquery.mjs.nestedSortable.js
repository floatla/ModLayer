	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		_mouseStop: function(event, noPropagation) {
			...
			if (this.beyondMaxLevels) {
				...
				var txt = (this.options.maxLevels == 1) 
						? "No se permiten niveles de profundidad" 
						: 'Solo se permiten '+ this.options.maxLevels +' niveles de profundidad.';

				window.appUI.RenderMessage(txt);
		...
		toArray: function(options) {
			...
			function _recursiveArray(item, depth, left) {
				...
				if (id) {
						ret.push({"item_id": id[2], "parent_id": pid, "order": $(item).index(), "depth": depth, "left": left, "right": right});
				}
